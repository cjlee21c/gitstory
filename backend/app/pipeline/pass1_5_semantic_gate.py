...
def _gate_candidate(candidate):
    ...
    candidates, so callers can run many of these concurrently."""
...
def pass_1_5_semantic_gate(qualified_candidates):
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        results = executor.map(_gate_candidate, qualified_candidates)
        elite_candidates = [r for r in results if r is not None]
